...
class PacmanGraphics:
    ...
    def drawAgentObjects(self, state):
        self.agentImages = []  # (player, image)
        for index, player in enumerate(state.players):
            if player.isPacman:
                image = self.drawPacman(player, index)
                self.agentImages.append((player, image))
            else:
                image = self.drawGhost(player, index)
                self.agentImages.append((player, image))
        refresh()
    ...
    def update(self, newState, player):
        # agentIndex = player.index
        self.draw(newState)
        sleep(0.3)
        # if self.agentImages[agentIndex][0].isPacman != player.isPacman:
        #     self.swapImages(agentIndex, player)
        # prevState, prevImage = self.agentImages[agentIndex]
        # if player.isPacman:
        #     self.animatePacman(player, prevState, prevImage)
        # else:
        #     self.moveGhost(player, agentIndex, prevState, prevImage)
        # self.agentImages[agentIndex] = (player, prevImage)

        # if newState._foodEaten != None:
        #     self.removeFood(newState._foodEaten, self.food)
        # if newState._capsuleEaten != None:
        #     self.removeCapsule(newState._capsuleEaten, self.capsules)
        # self.infoPane.updateScore(newState.pacmanScore)
    ...
    def getEndpoints(self, dir, p=(0,0)):
        return (0, 0)
    ...
    def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
        old_x, old_y = self.to_screen(prevGhost.pos)
        new_x, new_y = self.to_screen(ghost.pos)
