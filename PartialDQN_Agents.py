...
params = {
    # Model backups
    'load_file': './saves/model-partial_110213_468',
    'save_file': 'partial',
    'save_interval': 20000,
...
class PartialDQNAgent:
    ...
    def getStateMatrices(self, state):
        pos = self.player.pos 
        pos_s = to_screen_axis(pos, state.layout.height)
        ...
        def getWallMatrix(state):
            ...
            for i in range(height):
                for j in range(width) :
                    # (j, i) 对应在屏幕上的坐标
                    sx,sy=to_point_in_map(pos_s, (j, i), SIGHT)
        ...
        def getGhostMatrix(state):
            ...
            for player in state.players:
                if not player.isPacman:
                    if not player.scaredTimer > 0:
                        posG = player.pos
                        posG_s = to_screen_axis(posG, state.layout.height)
                        sx, sy = to_point_in_viewport(pos_s, posG_s, SIGHT)
                        if bounded((sx, sy), SIGHT, SIGHT) :
                            nx,ny = to_normal_axis((sx, sy), SIGHT)
                            matrix[nx][ny] = 1
        ...
        def getCapsulesMatrix(state):
            ...
            for i in capsules:
                sx,sy = to_screen_axis(i, state.layout.height)
                vx,vy = to_point_in_viewport(pos_s, (sx, sy), SIGHT)
                if bounded((vx, vy), SIGHT, SIGHT):  
                    nx,ny = to_normal_axis((vx, vy), SIGHT)
                    matrix[nx][ny] = 1
